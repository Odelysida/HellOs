...
;          ax = 1 (Success)                                   ;
;*************************************************************;
    mov ax, 2403h           ; Query A20 Gate Support (ie. later PS/2s systems)
    int 15h
    ...
    int 15h
    jb .failure             ; couldnt get status
    cmp ah, 0
    jnz .failure            ; couldnt get status
    cmp al, 1
    jz .success             ; A20 is already activated
    mov ax, 2401h           ; Enable A20 gate
    int 15h
    jb .failure             ; Couldnt enable A20 Gate
    cmp ah, 0
    jnz .failure            ; couldnt enable A20 Gate
   .success:
        mov ax, 1
        ret
   .failure:
